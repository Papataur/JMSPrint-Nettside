import { useState } from "react";import heroImage from "./heroimage.jpg";import "./style.css";import JMSPrint from "./logo.jpg";

import deskholder from "./deskholder.jpg";import deskholderBlack from "./deskholder-black.jpg";import deskholderBlue from "./deskholder-blue.jpg";import deskholderPink from "./deskholder-pink.jpg";import deskholderWhite from "./deskholder-white.jpg";

import holderpink from "./holderpink.jpg";import holderpinkBlack from "./holderpink-black.jpg";import holderpinkWhite from "./holderpink-white.jpg";import holderpinkGrey from "./holderpink-grey.jpg";import holderpinkBlue from "./holderpink-blue.jpg";

import salttraktWhite from "./salttrakt.jpg";import salttraktBlack from "./salttrakt-black.jpg";import salttraktGrey from "./salttrakt-grey.jpg";import salttraktBlue from "./salttrakt-blue.jpg";import salttraktPink from "./salttrakt-pink.jpg";

import boksaapner from "./boksaapner.jpg";import snusboks from "./snusboks.jpg";import justerbarStraaledyse from "./justerbar-straaledyse.jpg";

function ProductItem({image,colorImages = {},title,price,text,onImageClick,onAddToCart,isFavorite,onToggleFavorite,}) {const [selectedImage, setSelectedImage] = useState(image);const [selectedColor, setSelectedColor] = useState("Ikke valgt");

const chooseColor = (color, img) => {if (!img) return;setSelectedColor(color);setSelectedImage(img);};
...
export default function App() {const [selectedImage, setSelectedImage] = useState(null);const [cart, setCart] = useState([]);const [customer, setCustomer] = useState({name: "",address: "",phone: "",email: "",});

const [showToast, setShowToast] = useState(false);const [checkoutError, setCheckoutError] = useState("");const [isPaying, setIsPaying] = useState(false);const [isSendingOrder, setIsSendingOrder] = useState(false);const [orderSuccess, setOrderSuccess] = useState(false);const [lastOrderNumber, setLastOrderNumber] = useState("");const [vippsSuccess, setVippsSuccess] = useState(false);const [favorites, setFavorites] = useState([]);

const addToCart = (product) => {setCart([...cart, product]);setShowToast(true);
...
const productTotal = cart.reduce((total, item) => {return total + parseInt(item.price);}, 0);

const totalPrice = productTotal + shippingPrice;

const toggleFavorite = (title) => {if (favorites.includes(title)) {setFavorites(favorites.filter((fav) => fav !== title));} else {setFavorites([...favorites, title]);}};

const validateCustomer = () => {if (!customer.name || !customer.address || !customer.phone || !customer.email) {setCheckoutError("Fyll inn alle feltene");return false;}
...
Varer: {productTotal} krFrakt: {shippingPrice} kr
